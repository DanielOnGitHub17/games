...
class Player {
    ...
    remove() {
        ...
        sound[6].play(); container.removeChild(this.you);
        delete players[players.indexOf(this)]
    }
}
class Enemy extends Player {
    constructor(where, angle) {
        ...
        random(2) == 1 ? this.dang = 90 : this.dang = -90;
    }
    update() {
        this.yourHead.textContent = this.life;
        if (this.life <= 0) {
            this.remove(); enemies = enemies.filter(i => i); return;
        }
        if (this.target) {
            let ang = (this.target.center.y - this.center.y) / (this.target.center.x - this.center.x)
            ang = (Math.atan(ang) / Math.PI) * 180
            this.angle = ang + this.dang; setRotation(this.you, this.angle);
            return 50;
        } else {
            this.angle = random(360);
            setRotation(this.you, this.angle); return 1000;
        }
    }
    shoot() {
        sound[1].play();
    ...
    remove() {
        sound[5].play()
        container.removeChild(this.you);
